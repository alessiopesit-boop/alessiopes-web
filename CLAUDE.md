...
- Tag fissi (og:image, og:site_name, JSON-LD `ProfessionalService` + `Person`) in `index.html`. Il `ProfessionalService` (`@id` `#business`) e la `Person` (`@id` `#alessio`, E-E-A-T: `jobTitle`, `knowsAbout`, `sameAs` Trustpilot/GitHub) sono collegati via `@id` (`founder`/`worksFor`). Le certificazioni vanno in `hasCredential` sulla `Person` quando confermate (nome, ente, anno). `robots.txt`, `sitemap.xml` e `og-image.png` (1200x630) in `public/`.
...
- Foto: **attiva in `/chi-sono`** (headshot 1:1 `public/alessio.webp`; lo slot `.about-photo` è reso quadrato via `.about-photo img` in `styles.css`, e la griglia torna a 2 colonne via `:has`). In `home.html` lo slot resta **nascosto** (`@if(false)`) per scelta. Per cambiare/rimuovere la foto: `chi-sono.html` + `public/alessio.webp`.
...
- **Recensioni**: sezione "Dicono di me" (`home.html`, `@if (true)`) ATTIVA con recensioni **vere** da **Trustpilot** (non più Google/finte). Hardcoded (niente widget/API di terze parti: si romperebbe il cookieless e peserebbe sul LCP), con lieve pulizia di punteggiatura (non verbatim assoluto) + badge "verificata su Trustpilot" che linka **alla singola recensione** (profilo: `it.trustpilot.com/review/alessiopes.it`). Il nome recensore ha il ruolo/attività linkata a `/progetti` dove serve (es. Gavino Masala = titolare General Services 2013). La griglia `.reviews-grid` è a **2 colonne su desktop** (1 su mobile): con 1 sola recensione la card resta a sinistra e il posto per la seconda è a destra. Per aggiungerne: una nuova `.review` nella griglia; valutare un link "tutte le recensioni su Trustpilot" quando saranno ≥2.
